...
function Card({ item }) {
  return (
    <div className="flex flex-col gap-6 p-4">
      {/* Card container */}
      {/* <div className="flex flex-col md:flex-row gap-4 border border-gray-300 rounded-lg shadow-md bg-white w-full"> */}
      <motion.div
        className="flex flex-col md:flex-row gap-4 border border-gray-300 rounded-lg shadow-md bg-white w-full"
      ...
      >
        ...
        <Link to={`/${item.id}`} className="flex shrink-0">
          <img
            src={item.img}
            alt={item.title}
            className="w-full md:w-64 h-40 md:h-40 rounded-lg object-cover cursor-pointer"
          />
        </Link>
